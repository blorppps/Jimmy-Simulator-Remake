...
class jimmy:
    ...
    def __init__(self,position,type,spawnflags=[]):
        self.position = position
        ...
        if 'low-health' in self.spawnflags:
            self.health = 2
            self.maxhealth = 2
    ...
    def healthbar(self):
        ...
        if 'large-healthbar' in self.spawnflags: #how thick the bar is
            width = 7
        else:
            width = 5
...
while mainloop:
    ...
    for enemy in enemies:
            ...
            if enemy.knockback == (0,0):
                ...
                if math.dist(enemy.rect.center,player.rect.center) < 600:
                    ...
                    if enemy.attacktimer == 0:
                        ...
                        if random.choice((True,False)) and len(enemies) < 4: #summon attack
                            #dont summon enemies if there are too many currently spawned
                            for coords in [(100,0),(0,100),(-100,0),(0,-100)]:
                                enemies.append(jimmy(position=(enemy.position[0]+coords[0],enemy.position[1]+coords[1]),type='blue-laser',spawnflags=['low-health']))
                                
                        else: #special laser attack
                            for coords in [(0,0),(50,0),(0,50),(-50,0),(0,-50),(100,0),(0,100),(-100,0),(0,-100)]:
        ...
        if not enemy.rect.colliderect(player.rect): #this stops enemies from being pushed onto the players hitbox
            for collide in enemies:
                if enemy.rect is collide.rect:
                    pass
                elif enemy.rect.colliderect(collide.rect):
                    slope = math.atan2(enemy.position[1] - collide.position[1],enemy.position[0] - collide.position[0])
                    distance = 1 / (math.dist(enemy.position,collide.position) / 40)
                    enemy.position = (enemy.position[0]+math.cos(slope)*distance,enemy.position[1]+math.sin(slope)*distance)

        #despawning
        if math.dist(enemy.position,player.position) > 700:
            enemy.despawntimer = enemy.despawntimer + 1
            if enemy.despawntimer > 240:
        ...
        if enemy.health == 0 or enemy.health < 0:
            ...
            if not 'low-health' in enemy.spawnflags:
